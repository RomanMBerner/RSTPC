...
BcHistoFitterFast::BcHistoFitterFast(TH1D * hist, TF1 * func):BCHistogramFitter(hist,func)
{
	if( !(hist && func) ) exit(-1);
...
double BcHistoFitterFast::LogLikelihood(const std::vector<double> & parameters)
{
